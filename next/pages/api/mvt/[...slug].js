...
function validateIslandPointsFilters(
  id_toponim,
  nammap,
  artinam,
  sjhnam,
...
export default async function mvtHandler(req, res) {
  ...
  if (layerName === "titik-pulau") {
    ...
    ({ whereQuery, extraParams } = validateIslandPointsFilters(
      id_toponim,
      nammap,
      artinam,
      sjhnam,
  ...
  } else if (layerName === "garis-pantai") {
    tableName = "garis_pantai";
    featureIdCol = "fid";
    whereQuery = "";
    extraParams = [];
  } else {
    return res.status(400).json({ message: "Layer tidak valid" });
  }
